...
class MarkdownDocumenter {
    ...
    _writeInterfaceTables(output, apiClass) {
        ...
        if (eventsTable.rows.length > 0) {
            ...
            for (let item of events)
                this._writeApiItem(item, output);
        }
        if (propertiesTable.rows.length > 0) {
            output.appendNode(new DocHeading_1.DocHeading({
                configuration: this._tsdocConfiguration,
    ...
    _createTitleCell(apiItem, originalItem) {
        const configuration = this._tsdocConfiguration;
    ...
    _createPropertyTypeCell(apiItem) {
        ...
        if (apiItem instanceof api_extractor_model_1.ApiPropertyItem) {
            section.appendNodeInParagraph(new tsdoc_1.DocCodeSpan({
                configuration,
    ...
    _getLinkFilenameForApiItem(apiItem, fromApiItem) {
