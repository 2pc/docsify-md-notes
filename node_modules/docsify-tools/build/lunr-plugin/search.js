...
// let INDEXS: { [key: string]: any } = {};
// declare let Docsify: any;
...
function saveData(maxAge, expireKey, indexKey) {
    localStorage.setItem(expireKey, (Date.now() + maxAge).toString());
    ...
    tokens.forEach((token) => {
        if (token.type === "heading" && token.depth <= depth) {
            ...
            if (!index[slug]) {
                ...
                if (!token.text) {
                    if (token.type === "table") {
                        token.text = token.cells.map((rows) => rows.join(" | ")).join(" |\n ");
                    }
                }
...
function search(query) {
    ...
    if (keywords.length !== 1) {
        keywords = [query].concat(keywords);
    }
...
async function init(config, vm) {
    ...
    const isExpired = Number(localStorage.getItem(expireKey) || "0") < Date.now();
